...
def bootstrap(seeds, instances, iterations=5, threshold=2.0):
    """
    Perform bootstrapping with given seeds and instances.
    """
    for iteration in range(iterations):
        # 1. Build feature counts from current seeds
        feature_counts = count_features(seeds)

        # 2. Build decision list
        decision_list = create_decision_list(feature_counts)

        # 3. Label all instances
        new_labels = []
        for inst in instances:
            sense, llr = label_instance(inst, decision_list)
            if sense is not None and abs(llr) >= threshold:
                new_labels.append((inst, sense))

        print(f"High-confidence new labels: {len(new_labels)}")

        # 4. Add new labels to seed set
        # Avoid duplicates
        before = len(seeds)
        seeds = seeds + [lbl for lbl in new_labels if lbl not in seeds]
        after = len(seeds)

        print(f"Seeds before: {before}, after: {after}")

        # If nothing new added, stop
        if before == after:
            print("No new seeds — converged")
            break

    return seeds, decision_list
